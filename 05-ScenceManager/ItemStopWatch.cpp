...
CItemStopWatch::CItemStopWatch()
{
	this->visible = false;
}

void CItemStopWatch::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
...
void CItemStopWatch::Render()
{
	animation_set->at(0)->Render(x, y, 1);
}

void CItemStopWatch::GetBoundingBox(float& left, float& top, float& right, float& bottom)
{
	left = x;
	top = y;
