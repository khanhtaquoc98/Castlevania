...
*/
void CGame::Draw(int x, int y, int nx, LPDIRECT3DTEXTURE9 texture, int left, int top, int right, int bottom, int alpha)
{
...
bool CGame::InCamera(LPGAMEOBJECT obj)
{
	float objLeft, objRight, objTop, objBottom;
	obj->GetBoundingBox(objLeft, objTop, objRight, objBottom);

	//Obj Width = objRight - objLeft
	//ObjHeight = objBottom - objTop

	if(obj->x + objRight - objLeft < this->cam_x || obj->x > this->cam_x + screen_width 
		|| obj->y > this->cam_y + screen_height || obj->y + objBottom - objTop < this->cam_y)
		return false;
	return true;
}
...
#define GAME_FILE_SECTION_OBJECTS			7
