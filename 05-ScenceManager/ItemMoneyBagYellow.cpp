...
CItemMoneyBagYellow::CItemMoneyBagYellow()
{
	this->visible = false;
}

void CItemMoneyBagYellow::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
...
void CItemMoneyBagYellow::Render()
{
	animation_set->at(0)->Render(x, y, 1);
}

void CItemMoneyBagYellow::GetBoundingBox(float& left, float& top, float& right, float& bottom)
{
	left = x;
	top = y;
